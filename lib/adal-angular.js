        ...
        AdalModule.provider('adalAuthenticationService', function () {
            ...
            this.init = function (configOptions, httpProvider) {
                if (configOptions) {
					...
					if (typeof configOptions.enabled === 'undefined') {
						configOptions.enabled = true;
					}
        ...
        AdalModule.factory('ProtectedResourceInterceptor', ['adalAuthenticationService', '$q', '$rootScope', function (authService, $q, $rootScope) {
            ...
            return {
                request: function (config) {
					...
					if (config) {
						if (typeof authService.config.enabled !== 'undefined' && authService.config.enabled === false) {
							authService.info('Skipping ADAL authentication for url ' + config.url + ' because ADAL is disabled');
							return config;
						}
